...
import { ConvertKToC, ConvertPop, ConvertUnixTimeToUTC, ConvertWindDeg, ConvertWindSpeed } from '../utils/index';
            ...
            <Layout style={[styles.flexRow, { alignItems: 'center', marginBottom: 4 }]}>
                ...
                <Layout style={{ flex: 8 }}>
                    ...
                    <Text style={[styles.textStyle]}>
                        {ConvertKToC(data?.temp)}&#176;C - Cảm Giác Như: {ConvertKToC(data?.feels_like)}&#176;C
                    </Text>
            ...
            <Layout style={[styles.flexRow, { alignItems: 'flex-start', justifyContent: 'center' }]}>
                ...
                <Layout style={{ flex: 8 }}>
                    <Text style={{ textTransform: 'capitalize' }}>
                        Gió: {ConvertWindSpeed(data?.wind_speed)} km/h, {ConvertWindDeg(data?.wind_deg)}{' '}
                    </Text>
...
const HourlyPage = () => {
    ...
    if (isThreeDays) {
        todayData = filter(hourlyData, (value) => moment(value.dt * 1000).dayOfYear() === moment().dayOfYear());
        tomorrowData = filter(hourlyData, (value) => moment(value.dt * 1000).dayOfYear() === moment().dayOfYear() + 1);
        afterTomorrowData = filter(hourlyData, (value) => moment(value.dt * 1000).dayOfYear() === moment().dayOfYear() + 2);
    } else {
        todayData = filter(hourlyData, (value) => moment(value.dt * 1000).dayOfYear() === moment().dayOfYear());
        tomorrowData = filter(hourlyData, (value) => moment(value.dt * 1000).dayOfYear() === moment().dayOfYear() + 1);
        afterTomorrowData = [];
            ...
            <Section>
                ...
                <SectionBody>
                    <ScrollView>
                        ...
                        <Section>
                            <SectionTitle>{ConvertUnixTimeToUTC(afterTomorrowData[0]?.dt, 'dddd, Do MMMM')}</SectionTitle>
                            <SectionBody>
